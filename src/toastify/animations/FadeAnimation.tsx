...
export function FadeAnimation({
  ...
  const targetY = (isTop ? 1 : -1) * index * 66;
  ...
  return (
    <motion.article
      ...
      aria-atomic="true"
      initial={{ opacity: 0, scale: 0.9, y: targetY, filter: "blur(4px)" }}
      animate={{
      ...
      exit={{ opacity: 0, scale: 0.9, filter: "blur(4px)", transition: { duration: 0.22, ease: "easeOut" } }}
      transition={{ type: "spring", ...springConfig }}
      drag="x"
      dragConstraints={{ left: 0, right: 0 }}
      dragElastic={0.6}
      onDragEnd={(_, info) => {
        if (Math.abs(info.offset.x) > 60 || Math.abs(info.velocity.x) > 500) onDismiss();
      }}
      className="toastify-item absolute h-14 w-full select-none cursor-grab active:cursor-grabbing"
      style={{ [isTop ? "top" : "bottom"]: 0, left: 0, right: 0 }}
    >
      {children}
    </motion.article>
  );
